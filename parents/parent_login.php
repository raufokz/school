...
// Prepare the SQL statement to find the parent
...
if (mysqli_num_rows($result) > 0) {
    $parent = mysqli_fetch_assoc($result); // Fetch the parent's data

    // Verify the password
    if (password_verify($pass, $parent['password'])) {
        // Store parent's ID and email in the session
        $_SESSION['parent_id'] = $parent['id'];
        $_SESSION['parent_email'] = $parent['email'];
        $_SESSION['parent_name'] = $parent['first_name'] . ' ' . $parent['last_name'];
        header("location:dashboard.php"); // Redirect to parent dashboard
